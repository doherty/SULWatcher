			...
			if len(a) == 2:
				...
				if self.getCloak(e.source()) in config.get('Setup', 'privileged').split('<|>'):
					try:
						...
						self.msg('You have to follow the proper syntax. See \x0302[[tswiki:User:Mike.lifeguard/SULWatcher]]\x03.', nick)
			...
			if len(a) == 2:
				...
				if self.getCloak(e.source()) in config.get('Setup', 'privileged').split('<|>'):
					try:
						...
						print 'Error: %s' % sys.exc_info()[1]
						self.msg('You have to follow the proper syntax. See \x0302[[tswiki:User:Mike.lifeguard/SULWatcher]]\x03.', target)
